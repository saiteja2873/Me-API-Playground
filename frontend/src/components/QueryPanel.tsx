...
export default function QueryPanel() {
  ...
  useEffect(() => {
    fetch("https://me-api-playground-3-hlsg.onrender.com/query/skills/top")
      .then((res) => res.json())
      .then(setTopSkills)
  ...
  const handleSkillSearch = async (skillName?: string) => {
    ...
    try {
      const res = await fetch(
        `https://me-api-playground-3-hlsg.onrender.com/query/projects?skill=${s}`
      );
  ...
  const handleQuerySearch = async () => {
    ...
    try {
      const res = await fetch(
        `https://me-api-playground-3-hlsg.onrender.com/query/search?q=${encodeURIComponent(query)}`
      );
